...
Dit probleem kan meerdere oorzaken hebben, zelfs als 'feature info' (objectinformatie) correct wordt weergegeven wanneer u klikt.

Een veelvoorkomende oplossing is het controleren van de **configuratie van de laag**. In een bekend geval werd het probleem opgelost door een specifiek **selectievakje in de laagconfiguratie-instellingen** aan te vinken dat de 2D-zichtbaarheid regelde. Controleer alle instellingen voor zichtbaarheid en modi voor die laag.
...
Een **"Cross-Origin Resource Sharing" (CORS)** fout treedt op wanneer een webapplicatie (bijv. `https://hub.clearly.app`) gegevens probeert op te vragen van een server op een ander domein (bijv. `https://www.gis.example.nl/gisweb2/wms`). De server blokkeert dit standaard uit veiligheidsoverwegingen.
...
De server `www.gis.example.nl` moet bijvoorbeeld worden bijgewerkt om 'origins' zoals `https://hub.clearly.app` toe te staan.
